...
const numDocs = classes.A.length + classes.B.length;
let unitedDoc = Object.keys(classes).reduce((acc, aClass) => {
    return acc + classes[aClass].reduce((acc, doc) => acc + doc, '') + ' ';
}, '');
...
const statsByClass = Object.keys(classes).reduce((acc, aClass) => {
    let numDocsOfClass = classes[aClass].length;
    acc.logprior[aClass] = Math.log(numDocsOfClass / numDocs);

    let unitedDocOfClass = classes[aClass].reduce((acc, doc) => acc + doc, '');
